...
public class DefaultBoardDao {
  ...
  public DefaultBoardDao(DataInputStream in, DataOutputStream out) {
    this.in = in;
    this.out = out;
  }

  public DefaultBoardDao(List<Board> list) {
    this.list = list;
  }
  ...
  public void load(String filename) {
    ...
    try (BufferedReader in = new BufferedReader(new FileReader(filename))) {
      ...
      // 1. JSON 데이터를 어떤 타입의 객체로 변환할 것인지 그 타입정보를 준비한다.
