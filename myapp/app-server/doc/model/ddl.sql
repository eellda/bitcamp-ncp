...
CREATE TABLE lms_teacher (
  ...
  fulltime   INTEGER NOT NULL COMMENT '전입여부', -- 전입여부
...
CREATE TABLE lms_class (
  ...
  copacity      INTEGER     NOT NULL COMMENT '수용인원' -- 수용인원
)
COMMENT '강의실';
...
-- 기본주소 유니크 인덱스
CREATE UNIQUE INDEX UIX_lms_addr
  ON lms_addr ( -- 기본주소
    postno ASC -- 우편번호
  );
...
CREATE TABLE lms_application (
  ...
  application_state_id VARCHAR(10) NULL     COMMENT '신청상태' -- 신청상태
)
COMMENT '수강신청';
...
CREATE TABLE lms_application_state (
  application_state_id VARCHAR(10) NOT NULL COMMENT '신청상태', -- 신청상태
