...
// 웹 애플리케이션이 시작/종료 될때 실행되는 객체

@WebListener 
// 서블릿 컨테이너에게 이 클래스가 리스너 구현체임을 알려줘야함
// 그래야만 서블릿 컨테이너는 이 클래스의 인스턴스를 생성한다.
// 그리고 웹 앱이 시작 되거나 종료 될 때 메서드를 호출해 준다.
public class ContextLoaderListener implements ServletContextListener {

  @Override
  public void contextInitialized(ServletContextEvent sce) {
    // 웹 애플리케이션이 시작될 때 서블릿 컨테이너가 호출된다.
    ...
    try {
      ...
      System.out.println("web Application 자원을 준비하는 중에 오류 발생!");
      e.printStackTrace();
    }
  }
  ...
  public void contextDestroyed(ServletContextEvent sce) {
    // 웹 어플리케이션이 종료될 때 서블릿 컨테이너가 호출된다
